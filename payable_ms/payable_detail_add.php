...
//   	$objResponse->alert("formData: " . print_r($_POST, true));
	...
	if (!$bError) {
		...
		//檢查物料編碼是否重覆
		$Qry="SELECT material_no FROM payable_detail WHERE purchase_order_id = '$purchase_order_id' AND material_no = '$material_no'";
		$mDB->query($Qry);
		$total = $mDB->rowCount();
		if ($total > 0) {
			$mDB->remove();
		...
		$Qry="insert into payable_detail (purchase_order_id,seq,work_project,material_no,warehouse,purchase_qty,unit_price,remarks,last_modify) values ('$purchase_order_id','$seq','$work_project','$material_no','$warehouse','$purchase_qty','$unit_price','$remarks',now())";
		$mDB->query($Qry);

        $mDB->remove();

		$objResponse->script("parent.payable_detail_myDraw();");
...
$mDB = "";
$mDB = new MywebDB();
$Qry="select purchase_order_id,auto_seq from payable where auto_seq = '$auto_seq'";
$mDB->query($Qry);
if ($mDB->rowCount() > 0) {
...
$ajax_get_inventory = $m_location."/sub_modal/project/func09/payable_ms/ajax_get_inventory.php";
